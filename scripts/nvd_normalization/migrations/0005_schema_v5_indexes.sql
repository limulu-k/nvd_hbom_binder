...
CREATE INDEX identity_node_scope_vendor_idx
    ON identity_node(scope_kind,vendor_key,product_key,part);
CREATE INDEX identity_key_lookup_idx
    ON identity_key(key_kind,key_value,node_id);
CREATE INDEX identity_alias_status_idx
    ON identity_alias_edge(status,alias_class,evidence_tier);
CREATE INDEX identity_alias_left_status_idx
    ON identity_alias_edge(status,left_node_id);
CREATE INDEX identity_alias_right_status_idx
    ON identity_alias_edge(status,right_node_id);
CREATE INDEX identity_cluster_scope_idx
    ON identity_cluster(scope_kind,review_state,strict_eligible);
CREATE INDEX identity_cluster_member_product_idx
    ON identity_cluster_member(product_id,cluster_id);
